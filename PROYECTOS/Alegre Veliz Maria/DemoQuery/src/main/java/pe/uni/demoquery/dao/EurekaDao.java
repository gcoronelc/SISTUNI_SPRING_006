...
public class EurekaDao extends AbstractDaoSupport implements EurekaDaoSpec{
	...
	@Transactional( propagation=Propagation.REQUIRES_NEW,
			rollbackFor=Exception.class)
	@Override
	public void procDeposito(String cuenta, Double importe, String clave, String empleado) {
		String SQL;
		Integer conta;
		// Verificar la cuenta
		SQL = "SELECT COUNT(1) FILAS  "
	...
	public EmpleadoDto getEmpleado(String codigo) {
		...
		String sql = "SELECT CHR_EMPLCODIGO, VCH_EMPLPATERNO, "
				...
				+ " FROM EUREKA.EMPLEADO WHERE CHR_EMPLCODIGO = ?";
		EmpleadoDto dto = jdbcTemplate.queryForObject(sql, new EmpleadoMapper(), codigo);
		return dto;
	
	}

	@Override
	public List<EmpleadoDto> getEmpleado2(String codigo, String paterno, String materno, String nombre) {
		// Consulta
					String sql = "SELECT "
							+ "CHR_EMPLCODIGO codigo, VCH_EMPLPATERNO paterno, "
							+ "VCH_EMPLMATERNO materno, VCH_EMPLNOMBRE nombre "
							+ "FROM  EUREKA.EMPLEADO "
							+ "WHERE CHR_EMPLCODIGO = NVL2(?,?,CHR_EMPLCODIGO) "
							+ "AND   VCH_EMPLPATERNO LIKE NVL2(?,? || '%',VCH_EMPLPATERNO) "
							+ "AND   VCH_EMPLMATERNO LIKE NVL2(?,? || '%',VCH_EMPLMATERNO) "
							+ "AND   VCH_EMPLNOMBRE LIKE NVL2(?,? || '%',VCH_EMPL"
							+ "NOMBRE) "; 
					// Parametros
					Object[] parametros = {codigo, codigo, paterno, paterno, materno, materno, nombre, nombre};
					// La consulta
					List<EmpleadoDto> lista;
					lista = jdbcTemplate.query(sql, new BeanPropertyRowMapper(EmpleadoDto.class), parametros);
					return lista;
	}
