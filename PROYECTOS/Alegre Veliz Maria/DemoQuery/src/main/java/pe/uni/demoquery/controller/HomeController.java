...
public class HomeController {
	...
	@RequestMapping(value = "/ConsultarEmpleados.htm", method = RequestMethod.GET)
	public String ConsultarEmpleados() {	
		return "ConsultarEmpleados";
	}
	
	@RequestMapping(value = "/ConsultarEmpleadosResp3.htm", method = RequestMethod.GET)
	public @ResponseBody String ConsultarEmpleadosResp3(
		...
		// Proceso
		List<EmpleadoDto> lista = eurekaService.getEmpleado2(codigo, paterno, materno, nombre);
		String cadena = EGCCUtil.ListEmpleadosToString(lista);
		return cadena;
	}
	...
	@RequestMapping(value = "/DepositoProc.htm", method = RequestMethod.POST)
	public @ResponseBody String DepositoProc(
			@RequestParam("cuenta") String cuenta,
			@RequestParam("importe") Double importe,
			@RequestParam("clave") String clave			
			) {	
		String respuesta = "OK";
		try {
			this.eurekaService.procDeposito(cuenta, importe, clave, "0003");
			respuesta = "Proceso ejecutado correctamente.";
		} catch (Exception e) {
			respuesta = e.getMessage();
		}
